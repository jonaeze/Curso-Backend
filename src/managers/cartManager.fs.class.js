...
//POST
    ...
    addProductToCart = async (cartId, productId, quantity) => {
        try {
            if ((!Number.isInteger(cartId) && !Number.isInteger(productId)) || (cartId <= 0 && productId <= 0)) {
                throw new Error(`Los IDs deben ser números enteros y mayores que cero (0)`)
            }
            ...
            if (!Number.isInteger(quantity) || quantity <= 0) {
                throw new Error(`La cantidad debe ser un número entero y mayor que cero (0)`)
            }
            ...
            if (productIndex === -1) {
                ...
                if (quantity > product.stock) {
                    throw new Error('La cantidad solicitada supera el stock disponible del producto.');
                }
                // Agregar el producto al carrito
                carts[cartIndex].products.push({ id: productId, quantity });
            } else {
                const newQuantity = quantity + carts[cartIndex].products[productIndex].quantity // Se suma la cantidad solicitada nueva (quantity) con la cantidad ya en el carrito(carts[cartIndex].products[productIndex].quantity)
            ...
            await this.saveCarts(carts)
            return `El producto con ID ${productId} se agregó correctamente al carrito ${cartId}. Carrito actualizado: ${JSON.stringify(carts[cartIndex], null, 2)}`;
        } catch (error) {
            throw new Error(error.message)
        }
    }
