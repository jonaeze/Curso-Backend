...
class ProductsDaoFs {
    ...
    try {
      ...
      if (products.some((savedProduct) => savedProduct.code === product.code)) {
        throw new Error(
          "El código " +
            product.code +
            " ya se encuentra utilizado por otro producto."
        );
      }
    } catch (error) {
      throw new Error(error.message);
  ...
  getProductById = async (id) => {
    try {
      ...
      if (!product) {
        throw new Error(
          `Not Found: No se encontró el producto con el ID ${id}.`
        );
      }
      return product;
    } catch (error) {
  ...
  updateProduct = async (id, updatedFields) => {
    try {
      ...
      if (index !== -1) {
        const updatedProduct /*producto antiguo a updatear*/ = {
          id,
          ...updatedFields,
        };
  ...
  deleteProduct = async (id) => {
    try {
      ...
      if (index !== -1) {
        const newProducts /*Productos(ARRAY DE PRODUCTOS) sin el producto a borrar*/ =
